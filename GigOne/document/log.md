...
## 1. `server/services/gigiConversationService.js` — Gigi Conversational Reply Engine

**Created:** 2026-03-13

### What it does

This service generates Gigi's conversational replies. It takes the user's spoken text (already transcribed), their sentiment analysis, recent chat history, and optionally weather/traffic context — then asks Gemini to produce a short, empathetic Hinglish reply.
...
| **Prompt Engineering**         | A detailed system prompt defines Gigi's personality (warm, Hinglish, 2-3 sentences, empathetic)             |
| **Context window management**  | Only the last 6 conversation turns are included via `.slice(-6)` to stay within token limits                |
| **Optional context injection** | Weather & traffic data are conditionally appended to the prompt so Gigi can reference real-world conditions |

### Key Function

```js
generateReply(userText, sentiment, recentMessages, context);
// → Returns a string: Gigi's reply
```
...
- None yet. Will be tested end-to-end via `POST /api/gigi/chat`.
